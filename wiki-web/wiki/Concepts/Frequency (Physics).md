# Frequency (Physics)

---
**Summary**: All waves have three important features: amplitude, frequency, and wavelength. You can see these in Figure 1. - Amplitude is the distance from the middle of the wave to the top or bottom of the wave. - Frequency is the number of waves that pass a particular point per second. - Wavelength is the distance from one point on a wave to the same point on the next wave.
**Tags**: #science #student-book-7 #physics
**Created**: 2026-05-21
**Last Updated**: 2026-05-21
---

# Frequency (Physics)

All waves have three important features: amplitude, frequency, and wavelength. You can see these in Figure 1. - Amplitude is the distance from the middle of the wave to the top or bottom of the wave. - Frequency is the number of waves that pass a particular point per second. - Wavelength is the distance from one point on a wave to the same point on the next wave.

## Source

This concept is from the [Science Student Book 7](Science Student Book 7/Physics/Sound/2.1 Waves.md), Physics section.
