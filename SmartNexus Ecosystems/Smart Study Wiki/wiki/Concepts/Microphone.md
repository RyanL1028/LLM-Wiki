# Microphone

---
**Summary**: You can attach a microphone to an oscilloscope, like the one in The microphone produces an electrical signal that represents the sound wave. The wave that appears on the screen is transverse. However, the sound waves you make when you talk are longitudinal.
**Tags**: #science #student-book-7 #physics
**Created**: 2026-05-21
**Last Updated**: 2026-05-21
---

# Microphone

You can attach a microphone to an oscilloscope, like the one in The microphone produces an electrical signal that represents the sound wave. The wave that appears on the screen is transverse. However, the sound waves you make when you talk are longitudinal.

## Source

This concept is from the [Science Student Book 7](Science Student Book 7/Physics/Sound/2.3 Loudness and pitch.md), Physics section.
